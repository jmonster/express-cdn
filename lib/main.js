...
var checkStringIfModified = function(assets, fileName, S3, options, timestamp, callback) {
  ...
  return function(err, response) {
    ...
    // FORCE upload the css + files to CDN
    var type = mime.lookup(assets);

    if (response.statusCode === 200 && type !== 'text/css') {
      logger({ task: 'express-cdn', message: '"' + fileName + '" not modified and is already stored on S3' });
      return finishUpload();
    } else {
      logger({ task: 'express-cdn', message: '"' + fileName + '" was not found on S3 or was modified recently' });
        ...
        case 'text/css':
          ...
          readUtf8(options.publicDir + assets, function(error, body) {
            ...
            if (options.production) {
              ...
              /*
                This .replace does not actually modify the body because the result is not assigned to
                anything.  The value returned is unused, so we are basically creating an array of images
                that are to be uploaded.  True is used as a placeholder to ensure that each filename is
                only included in the images array once.
              */
            ...
            /*
              Process all of the functions in imagesToUpload, which will compile an upload all image assets
              to S3.  Once all images in the imagesToUpload array are complete, we call the callback function
              to replace all instances of the previous, local URL with the new S3 URL.
            */
            async.parallel(imagesToUpload, function() {
              ...
              compile(fileName, assets, S3, options, 'minify', type, null, finishUpload)(error, body);
            })
...
var processAssets = function(options, results, done) {
  ...
  if (options.endpoint) { s3options.endpoint = options.endpoint }
  ...
  async.map(results, function (result, iter) {
    var assets = result, type = '', fileName = '', position, timestamp = 0, hash = crypto.createHash("md5");;
...
var CDN = function(app, options, callback) {
  ...
  if (options.production && cluster.isMaster) {
    var walker = function () {
      var walker   = walk.walk(options.viewsDir)
        , results  = []
        , regexCDN = /CDN\(([^)]+)\)/g
        , regexHbs = /\{\{CDN src="([^"]+)"(?:\s(.*))?\}\}/g;

    var findCDNHelpers = function (root, stat, next) {
      ...
      if (ext === '.jade' || ext === '.ejs') {
        ...
        fs.readFile(path.join(root, stat.name), 'utf8', function(err, data) {
          if (err) throwError(err);
          var match;
          while( (match = regexHbs.exec(data)) ) {
            var result = ''
              , assets = match[1].replace(/(\r\n|\n|\r)/gm,'').trim().split(/\s*,\s*/)
            ...
            if (assetsLength > 1) {
              ...
              _.each(assets, function(asset, index) {
                result += '\'' + asset + '\'';
                if (index < assetsLength - 1) result += ', ';
              });
              result += ' ]';
            } else {
              result += '\'' + assets[0] + '\'';
            ...
            if (match[2]) {
              ...
              _.each(attributes, function(attribute, index) {
                attribute = attribute.match(/\s?([^=]+)="([^"]+)"/);
                result += '\'' + attribute[1] + '\': ' + '\'' + attribute[2] + '\'';
                if (index < attributesLength - 1) result += ', ';
              });
              result += ' }';
            }
            results.push(result);
      ...
      walker.on('symbolicLink', findCDNHelpers);
      walker.on('file', findCDNHelpers);
